...
<script>
if(${Staffs.status=='FAIL'}){
	 Swal.fire({
		  type: 'error',
		  title: '${Staffs.message}',
			...
			<div class="card">
				<div class="card-body">
							...
							<ul class="navbar-nav float-left mr-auto">
								...
								<li class="nav-item search-box">Search <a
									...
									<form class="app-search position-absolute"
										action="admin/staffs.htm">
										<input name="name_nv" type="text" class="form-control"
					...
					<div class="table-responsive">
					 <form action="admin/staffs.htm" id="formTable" method="Get">
						<table class="table table-hover">
							...
							<tbody>
								<c:forEach var="s" items="${Staffs.data.listItem}">
									<tr class="edit">
										<td>${s.id}</td>
										<td>${s.name}</td>
										<td>${s.depart}</td>
						...
						<input type="hidden" value="${Staffs.data.name }" id="name_nv" name="name_nv" />
					</form>
					</div>
				</div>
			</div>
...
<script type="text/javascript">
    ...
    $(function () {
        window.pagObj = $('#pagination').twbsPagination({
            totalPages: ${Staffs.data.totalPage} ,
            startPage: ${Staffs.data.startPage} ,
            visiblePages: 3,
